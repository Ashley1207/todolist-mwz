...
public class TasksIntegrationTests {
    ...
    @Autowired
    private MockMvc mockMvc;    //模拟MVC测试对象

    @Test
    @Order(1)   //此注解主要用于使测试的方法顺序执行,需要与@TestMethodOrder结合使用
    public void noParamTasksShouldReturnAllTasksFromService() throws Exception {

        this.mockMvc.perform(get("/api/tasks"))     //执行一个请求
                .andDo(print())     //添加一个结果处理器，打印整个响应结果信息
                .andExpect(status().isOk())     //验证状态码
                .andExpect(jsonPath("$[0].content").value("test"));//使用Json path验证JSON
    }
